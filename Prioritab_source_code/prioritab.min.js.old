//Prioritab_source_code/prioritab.min.js.old
...
function CheckDayCountdown() {
    if (document.querySelector("#workday-checkbox").checked) {
...
function CountdownDay() {
    ...
    document.getElementById('countdown-day-amount').innerHTML = `${String(prettyPCT)}%`;
}
function CountdownWorkday() {
    ...
    browser.storage.sync.get(['user-workday-start', 'user-workday-end'], function (retrieved) {
        const workdayStartString = retrieved['user-workday-start'] ?? "09:00";
        const workdayEndString = retrieved['user-workday-end'] ?? "18:00";
        ...
        if (now < workdayStart) {
            ...
            const totalDayMS = workdayEnd - workdayStart;
...
function CountdownMonthYear() {
    ...
    const totalYearMS = (isLeapYear ? 366 : 365) * totalDayMS;
    const yearProgressPCT = progressYearMS / totalYearMS * 100;
    const prettyYearPCT = Math.floor(yearProgressPCT);

    document.getElementById('countdown-month-amount').innerHTML = `${String(prettyMonthPCT)}%`;
    document.getElementById('countdown-year-amount').innerHTML = `${String(prettyYearPCT)}%`;
...
function bindFluidToCursor() {
    if (!fluid || !fluid.pointers || !fluid.pointers[0]) {
...
window.onload = function () {
    ...
    browser.storage.local.get(["user_bg_img"], function (result) {
        el.style.backgroundImage = `url("${result['user_bg_img']}")`;
    });
    ...
    browser.storage.sync.get(['user-workday-start', 'user-workday-end'], function (retrieved) {
        const workdayStart = retrieved['user-workday-start'] ?? "09:00";
        const workdayEnd = retrieved['user-workday-end'] ?? "18:00";
